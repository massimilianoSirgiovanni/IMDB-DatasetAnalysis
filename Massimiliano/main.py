...
selected = random.randint(0, len(graphImbd.nodes))
if selected in indexToActor:
    print(f"The node randomically selected is {selected} and it corrispond to the actor {indexToActor[selected]}")
else:
    print(f"The node randomically selected is {selected} and it corrispond to the movie {indexToMovie[selected]}")
print()
...
print(f"Movies without a release date in the dataset are {len(years[0])}")

if len(years[0]) != 0:
    print(f"First movie found with no release year is: {indexToMovie[next(iter(years[0]))]}")
print()
