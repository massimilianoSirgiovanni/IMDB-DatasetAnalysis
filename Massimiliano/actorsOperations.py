...
def actorParticFamousMovies(graph):
    ...
    for i in indexToActor:  # Iterate on all actors
        ...
        sum = sumNeighborList(graph, neighbors)  # Obtain the number of actor participating to each movie linked to i
        if sum > sumMax:    # Checking for the max value and the correspondent actor id
            sumMax = sum
            max = i
    if max > -1:           # Checking if there is no error
...
def manageOneCollaboration(actor1, actor2, actorGraph, dictEdge):
    # Manage the collaboration between two actors
    tmp = 0
    ...
    # If there is the node actor2 there are two possibilities: There is already an edge (actor1, actor2) or there isn't
        ...
        #if f"({actor1}, {actor2})" in dictEdge:
        tmp = dictEdge[f"({actor1}, {actor2})"]
        dictEdge[f"({actor1}, {actor2})"] = tmp + 1
        '''else:
            tmp = dictEdge[f"({actor2}, {actor1})"]
            dictEdge[f"({actor2}, {actor1})"] = tmp + 1'''
    else:
