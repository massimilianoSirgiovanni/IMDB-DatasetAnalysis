...
from yearsFunctions import years, yearsActor, extractYear
...
def addActor(graph, actor, idActor, year):
    ...
    if actor not in actorToIndex:
        ...
        if idActor not in yearsActor[year]:     # Add actor in years dictionary
...
def addMovie(graph, movie, idMovie, year):
    ...
    if movie not in movieToIndex:
        ...
        years[year].add(idMovie)                # Add actor in years dictionary
        idMovie = idMovie + 1
